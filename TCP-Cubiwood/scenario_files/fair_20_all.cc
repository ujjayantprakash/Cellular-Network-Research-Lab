...
Ptr<PacketSink> sink17;                 // packetsink application 
Ptr<PacketSink> sink18;
Ptr<PacketSink> sink19;
Ptr<PacketSink> sink20;
...
uint64_t lastTotalRx17 = 0;             /* The value of the last total received bytes */
uint64_t lastTotalRx18 = 0;             /* The value of the last total received bytes */
uint64_t lastTotalRx19 = 0;  
uint64_t lastTotalRx20 = 0; 
...
void
CalculateThroughput17 ()
{
  Time now = Simulator::Now ();                                         /* Return the simulator's virtual time. */


   
   cur = (sink17->GetTotalRx () - lastTotalRx17) * (double) 8 / 1e5;     /* Convert Application RX Packets to MBits. */
  std::cout << now.GetSeconds () << "s: \t" << cur << " Mbit/s" << std::endl;
...
//stream->GetStream () << Simulator::Now ().GetSeconds () << "\t" << oldCwnd << "\t" << newCwnd << std::endl;
  ...
  lastTotalRx17 = sink17->GetTotalRx ();
  Simulator::Schedule (MilliSeconds (100), &CalculateThroughput17);
  
}

void
CalculateThroughput18 ()
{
  Time now = Simulator::Now ();                                         /* Return the simulator's virtual time. */


   
   cur = (sink18->GetTotalRx () - lastTotalRx18) * (double) 8 / 1e5;     /* Convert Application RX Packets to MBits. */
  std::cout << now.GetSeconds () << "s: \t" << cur << " Mbit/s" << std::endl;
...
//stream->GetStream () << Simulator::Now ().GetSeconds () << "\t" << oldCwnd << "\t" << newCwnd << std::endl;
  ...
  lastTotalRx18 = sink18->GetTotalRx ();
  Simulator::Schedule (MilliSeconds (100), &CalculateThroughput18);
  
}

void
CalculateThroughput19 ()
{
  Time now = Simulator::Now ();                                         /* Return the simulator's virtual time. */


   
   cur = (sink19->GetTotalRx () - lastTotalRx19) * (double) 8 / 1e5;     /* Convert Application RX Packets to MBits. */
  std::cout << now.GetSeconds () << "s: \t" << cur << " Mbit/s" << std::endl;
...
//stream->GetStream () << Simulator::Now ().GetSeconds () << "\t" << oldCwnd << "\t" << newCwnd << std::endl;
  ...
  lastTotalRx19 = sink19->GetTotalRx ();
  Simulator::Schedule (MilliSeconds (100), &CalculateThroughput19);
  
}


void
CalculateThroughput20 ()
{
  Time now = Simulator::Now ();                                         /* Return the simulator's virtual time. */


   
   cur = (sink20->GetTotalRx () - lastTotalRx20) * (double) 8 / 1e5;     /* Convert Application RX Packets to MBits. */
  std::cout << now.GetSeconds () << "s: \t" << cur << " Mbit/s" << std::endl;
...
//stream->GetStream () << Simulator::Now ().GetSeconds () << "\t" << oldCwnd << "\t" << newCwnd << std::endl;
  ...
  lastTotalRx20 = sink20->GetTotalRx ();
  Simulator::Schedule (MilliSeconds (100), &CalculateThroughput20);
...
//	cmd.AddValue("interPacketInterval", "Inter-packet interval [us])", interPacketInterval);
  ...
  if(dcr)
  {
    path_to_res = "results/Scenario3/Cubic_dcr/twenty/";   
  }
  
  else
  {
    path_to_res = "results/Scenario3/Cubic/twenty/";   
  } 
...
ApplicationContainer sinkApps17;
ApplicationContainer sinkApps18;
ApplicationContainer sinkApps19;
ApplicationContainer sinkApps20;
  ...
  if (tcp)
    {
      ...
      uint16_t sinkPort17 = 20016;
      uint16_t sinkPort18 = 20017;
      uint16_t sinkPort19 = 20018;
      uint16_t sinkPort20 = 20019;
      ...
      Address sinkAddress17 (InetSocketAddress (ueIpIface.GetAddress (0), sinkPort17));
      Address sinkAddress18 (InetSocketAddress (ueIpIface.GetAddress (0), sinkPort18));
      Address sinkAddress19 (InetSocketAddress (ueIpIface.GetAddress (0), sinkPort19));
      Address sinkAddress20 (InetSocketAddress (ueIpIface.GetAddress (0), sinkPort20));
...
//Calle function e CalculateThroughput varaye mohasebe e throughput.
     ...
     //Simulator::Schedule (Seconds (0), &errorrate);
      ...
      sinkApps16.Stop (Seconds (simStopTime));
       ...
       PacketSinkHelper packetSinkHelper17 ("ns3::TcpSocketFactory", InetSocketAddress (Ipv4Address::GetAny (), sinkPort17));
//Badan ezafe kardam.
 packetSinkHelper17.SetAttribute ("Protocol", TypeIdValue (TcpSocketFactory::GetTypeId ()));
       sinkApps17 = packetSinkHelper17.Install (ueNodes.Get (0));
sink17 = StaticCast<PacketSink> (sinkApps17.Get (0));

      sinkApps17.Start (Seconds (0.));

//Calle function e CalculateThroughput varaye mohasebe e throughput.
     Simulator::Schedule (Seconds (0), &CalculateThroughput17);
     //Simulator::Schedule (Seconds (0), &errorrate);
     
      sinkApps17.Stop (Seconds (simStopTime));
      
      
       PacketSinkHelper packetSinkHelper18 ("ns3::TcpSocketFactory", InetSocketAddress (Ipv4Address::GetAny (), sinkPort18));
//Badan ezafe kardam.
 packetSinkHelper18.SetAttribute ("Protocol", TypeIdValue (TcpSocketFactory::GetTypeId ()));
       sinkApps18 = packetSinkHelper18.Install (ueNodes.Get (0));
sink18 = StaticCast<PacketSink> (sinkApps18.Get (0));

      sinkApps18.Start (Seconds (0.));

//Calle function e CalculateThroughput varaye mohasebe e throughput.
     Simulator::Schedule (Seconds (0), &CalculateThroughput18);
     //Simulator::Schedule (Seconds (0), &errorrate);
     
      sinkApps18.Stop (Seconds (simStopTime));
      
      
       PacketSinkHelper packetSinkHelper19 ("ns3::TcpSocketFactory", InetSocketAddress (Ipv4Address::GetAny (), sinkPort19));
//Badan ezafe kardam.
 packetSinkHelper19.SetAttribute ("Protocol", TypeIdValue (TcpSocketFactory::GetTypeId ()));
       sinkApps19 = packetSinkHelper19.Install (ueNodes.Get (0));
sink19 = StaticCast<PacketSink> (sinkApps19.Get (0));

      sinkApps19.Start (Seconds (0.));

//Calle function e CalculateThroughput varaye mohasebe e throughput.
     Simulator::Schedule (Seconds (0), &CalculateThroughput19);
     //Simulator::Schedule (Seconds (0), &errorrate);
     
      sinkApps19.Stop (Seconds (simStopTime));
      
       PacketSinkHelper packetSinkHelper20 ("ns3::TcpSocketFactory", InetSocketAddress (Ipv4Address::GetAny (), sinkPort20));
//Badan ezafe kardam.
 packetSinkHelper20.SetAttribute ("Protocol", TypeIdValue (TcpSocketFactory::GetTypeId ()));
       sinkApps20 = packetSinkHelper20.Install (ueNodes.Get (0));
sink20 = StaticCast<PacketSink> (sinkApps20.Get (0));

      sinkApps20.Start (Seconds (0.));

//Calle function e CalculateThroughput varaye mohasebe e throughput.
     Simulator::Schedule (Seconds (0), &CalculateThroughput20);
     //Simulator::Schedule (Seconds (0), &errorrate);
     
      sinkApps20.Stop (Seconds (simStopTime));
      ...
      TypeId tid9 = TypeId::LookupByName("ns3::TcpCubic");
      ...
      TypeId tid10 = TypeId::LookupByName("ns3::TcpCubic");
      ...
      TypeId tid11 = TypeId::LookupByName("ns3::TcpCubic");
      ...
      TypeId tid12 = TypeId::LookupByName("ns3::TcpCubic");
      ...
      TypeId tid13 = TypeId::LookupByName("ns3::TcpCubic");
      ...
      TypeId tid14 = TypeId::LookupByName("ns3::TcpCubic");
      ...
      TypeId tid15 = TypeId::LookupByName("ns3::TcpCubic");
      ...
      TypeId tid16 = TypeId::LookupByName("ns3::TcpCubic");
      ...
      TypeId tid17 = TypeId::LookupByName("ns3::TcpCubic");
      std::stringstream nodeId17;
      nodeId17 << remoteHostContainer.Get (0)->GetId();
      std::string specificNode17 = "/NodeList/" + nodeId17.str() + "/$ns3::TcpL4Protocol/SocketType";
      Config::Set(specificNode17, TypeIdValue(tid17));
      Ptr<Socket> ns3TcpSocket17 = Socket::CreateSocket(remoteHostContainer.Get (0), TcpSocketFactory::GetTypeId());
     
      Ptr<MyApp> app17 = CreateObject<MyApp> ();
      app17->Setup (ns3TcpSocket17, sinkAddress17, 1400, 5000000, DataRate ("1000Mb/s"));

      remoteHostContainer.Get (0)->AddApplication (app17);
      
      
      Ptr<OutputStreamWrapper> stream417 = asciiTraceHelper.CreateFileStream ((path_to_res + prefix_file_name + "-rtt17.txt").c_str());
      ns3TcpSocket17->TraceConnectWithoutContext ("RTT", MakeBoundCallback (&RttChange, stream417));
      
      app17->SetStartTime (Seconds (0.1));
      app17->SetStopTime (Seconds (stopTime));
      
      TypeId tid18 = TypeId::LookupByName("ns3::TcpCubic");
      std::stringstream nodeId18;
      nodeId18 << remoteHostContainer.Get (0)->GetId();
      std::string specificNode18 = "/NodeList/" + nodeId18.str() + "/$ns3::TcpL4Protocol/SocketType";
      Config::Set(specificNode18, TypeIdValue(tid18));
      Ptr<Socket> ns3TcpSocket18 = Socket::CreateSocket(remoteHostContainer.Get (0), TcpSocketFactory::GetTypeId());
     
      Ptr<MyApp> app18 = CreateObject<MyApp> ();
      app18->Setup (ns3TcpSocket18, sinkAddress18, 1400, 5000000, DataRate ("1000Mb/s"));

      remoteHostContainer.Get (0)->AddApplication (app18);
      
      
      Ptr<OutputStreamWrapper> stream418 = asciiTraceHelper.CreateFileStream ((path_to_res + prefix_file_name + "-rtt18.txt").c_str());
      ns3TcpSocket18->TraceConnectWithoutContext ("RTT", MakeBoundCallback (&RttChange, stream418));
      
      app18->SetStartTime (Seconds (0.1));
      app18->SetStopTime (Seconds (stopTime));
      
      
      TypeId tid19 = TypeId::LookupByName("ns3::TcpCubic");
      std::stringstream nodeId19;
      nodeId19 << remoteHostContainer.Get (0)->GetId();
      std::string specificNode19 = "/NodeList/" + nodeId19.str() + "/$ns3::TcpL4Protocol/SocketType";
      Config::Set(specificNode19, TypeIdValue(tid19));
      Ptr<Socket> ns3TcpSocket19 = Socket::CreateSocket(remoteHostContainer.Get (0), TcpSocketFactory::GetTypeId());
     
      Ptr<MyApp> app19 = CreateObject<MyApp> ();
      app19->Setup (ns3TcpSocket19, sinkAddress19, 1400, 5000000, DataRate ("1000Mb/s"));

      remoteHostContainer.Get (0)->AddApplication (app19);
      
      
      Ptr<OutputStreamWrapper> stream419 = asciiTraceHelper.CreateFileStream ((path_to_res + prefix_file_name + "-rtt19.txt").c_str());
      ns3TcpSocket19->TraceConnectWithoutContext ("RTT", MakeBoundCallback (&RttChange, stream419));
      
      app19->SetStartTime (Seconds (0.1));
      app19->SetStopTime (Seconds (stopTime));
      
      TypeId tid20 = TypeId::LookupByName("ns3::TcpCubic");
      std::stringstream nodeId20;
      nodeId20 << remoteHostContainer.Get (0)->GetId();
      std::string specificNode20 = "/NodeList/" + nodeId20.str() + "/$ns3::TcpL4Protocol/SocketType";
      Config::Set(specificNode20, TypeIdValue(tid20));
      Ptr<Socket> ns3TcpSocket20 = Socket::CreateSocket(remoteHostContainer.Get (0), TcpSocketFactory::GetTypeId());
     
      Ptr<MyApp> app20 = CreateObject<MyApp> ();
      app20->Setup (ns3TcpSocket20, sinkAddress20, 1400, 5000000, DataRate ("1000Mb/s"));

      remoteHostContainer.Get (0)->AddApplication (app20);
      
      
      Ptr<OutputStreamWrapper> stream420 = asciiTraceHelper.CreateFileStream ((path_to_res + prefix_file_name + "-rtt20.txt").c_str());
      ns3TcpSocket20->TraceConnectWithoutContext ("RTT", MakeBoundCallback (&RttChange, stream420));
      
      app20->SetStartTime (Seconds (0.1));
      app20->SetStopTime (Seconds (stopTime));
...
//
 ...
 AsciiTraceHelper ascii;
  ...
  totalPacketsThr = DynamicCast<PacketSink> (sinkApps17.Get (0))->GetTotalRx ();
  thr17 = totalPacketsThr * 8 / (simStopTime * 1000000.0); //Mbit/s
  std::cout << "  Rx Bytes: " << totalPacketsThr << std::endl;
  std::cout << "  Average Goodput For Seventeenth sink app: " << thr17 << " Mbit/s" << std::endl;
  totalPacketsThr = DynamicCast<PacketSink> (sinkApps18.Get (0))->GetTotalRx ();
  thr18 = totalPacketsThr * 8 / (simStopTime * 1000000.0); //Mbit/s
  std::cout << "  Rx Bytes: " << totalPacketsThr << std::endl;
  std::cout << "  Average Goodput For Eighteenth sink app: " << thr18 << " Mbit/s" << std::endl;
  totalPacketsThr = DynamicCast<PacketSink> (sinkApps19.Get (0))->GetTotalRx ();
  thr19 = totalPacketsThr * 8 / (simStopTime * 1000000.0); //Mbit/s
  std::cout << "  Rx Bytes: " << totalPacketsThr << std::endl;
  std::cout << "  Average Goodput For Nineteenth sink app: " << thr19 << " Mbit/s" << std::endl;
  totalPacketsThr = DynamicCast<PacketSink> (sinkApps20.Get (0))->GetTotalRx ();
  thr20 = totalPacketsThr * 8 / (simStopTime * 1000000.0); //Mbit/s
  std::cout << "  Rx Bytes: " << totalPacketsThr << std::endl;
  std::cout << "  Average Goodput For Twentieth sink app: " << thr20 << " Mbit/s" << std::endl;
  std::cout << std::endl << "*** TC Layer statistics ***" << std::endl;
  xSum=thr+thr2+thr3+thr4+thr5+thr6+thr7+thr8+thr9+thr10+thr11+thr12+thr13+thr14+thr15+thr16+thr17+thr18+thr19+thr20;
  avg=xSum/20;
  x2Sum=thr*thr+thr2*thr2+thr3*thr3+thr4*thr4+thr5*thr5+thr6*thr6+thr7*thr7+thr8*thr8+thr9*thr9+thr10*thr10+thr11*thr11+thr12*thr12+thr13*thr13+thr14*thr14+thr15*thr15+thr16*thr16+thr17*thr17+thr18*thr18+thr19*thr19+thr20*thr20;
  fairnessIndex=(xSum*xSum)/(x2Sum*20);
